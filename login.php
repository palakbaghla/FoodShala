...
if(isset($login))
{
	$query=mysqli_query($con,"select * from user where email='$email' && password='$password' where role_id=1");
    if($row=mysqli_fetch_array($query))
	{
		$customer_email =$row['email'];
		$_SESSION['user_id']=$customer_email;
		if(!empty($customer_email && $product_id))
		{
			 //$_SESSION['product']=$product_id;
			echo $_SESSION['user_id']=$customer_email;
...
if(isset($register))
{
	$query=mysqli_query($con,"select * from user where email='$email' role_id=1");
	...
	{
		if(mysqli_query($con,"insert into user (name,email,password,mobile) values('$name','$email','$password','$mobile')"))
    {
